        ...
        <div className="grid grid-cols-2 gap-4 p-4">            
            {datasets.map((dataset) => (
                <div key={dataset.label} className="p-2 flex justify-start items-center">
                    <div>
                    <Icon icon="circle"  className="text-[{borderColor}] text-xl cursor-pointer mr-0" style={{ color: dataset.borderColor }} />
                    </div>
                    <div className="text-xl mb-0 pl-2">{dataset.label}</div>
                </div>
            ))}
        </div>
