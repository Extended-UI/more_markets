    ...
    <div className='more-bg-secondary rounded-[12px] h-full w-full px-8'>
      <form onSubmit={handleSubmit}>
        ...
        <div className="flex flex-row justify-between mt-4 items-center">
          <div className="flex gap-2 text-l mb-5   items-center"> <span className="more-text-gray">Curator:</span><IconToken className='w-6 h-6' tokenName='abt' ></IconToken> <span >{curator}</span></div>
          <div className="flex gap-2 text-l mb-5 "><span className="more-text-gray">Liquidation LTV:</span> <FormatTwoPourcentage value={90} value2={125} ></FormatTwoPourcentage> </div>
        </div>
        ...
        <div className="flex justify-end py-5  rounded-b-[20px] ">
          <div className='mr-5'><MoreButton className='text-xl  py-4' text="Cancel" onClick={closeModal} color="gray" /></div>
          <MoreButton className='text-xl  py-4' text="Confirm" onClick={() => handleWithdraw()} color="primary" />
        </div>              
      </form>
    </div>
  );
