...
const VaultRepayTransaction: React.FC<Props> = ({ title, token, balance, apy, ltv, totalRepay, totalTokenAmount, curator, amount, validRepay, closeModal }) => {
  ...
  return (
    <div className='more-bg-secondary rounded-[12px] h-full w-full px-8 pb-4'>
      <form onSubmit={handleSubmit}>
        ...
        <div className='more-bg-primary px-8 rounded-t-[5px] '>
          <TokenAmount title="Repay" token={token} amount={amount} ltv={ltv} totalTokenAmount={totalTokenAmount} />
          <div className="flex flex-row justify-between items-center h-20">
            <div className='text-l'>Projeted rate</div>        
            <div className='flex  items-center gap-2'>
               <FormatPourcentage value={apy} />    <ArrowLongRightIcon className='w-4 h-4 text-grey' ></ArrowLongRightIcon>   <FormatPourcentage value={apy} />      
            </div>
          </div>
        </div>   
        ...
        <div className='more-bg-primary rounded-b-[5px] mt-[1px] px-8 flex flex-col gap-4 py-8'>          
          ...
          <div className="flex flex-row justify-between items-center ">
            <div className='text-l'>Collateral</div>        
            <div className='flex  items-center gap-2'>
               <FormatPrice value={balance} token={token}  ></FormatPrice>      
            </div>
          </div>
          
          <PositionChangeToken title='Borrow' token={token} value={balance} value2={balance} ></PositionChangeToken>

          <div className="flex flex-row justify-between items-center ">
           <div className='text-l'>LTV / Liquidation LTV</div>        
            <div className='flex  items-center gap-2'>
               <FormatPourcentage value={apy} />    <ArrowLongRightIcon className='w-4 h-4 text-grey' ></ArrowLongRightIcon>   <FormatTwoPourcentage value={apy} value2={apy} />      
            </div>
          </div>

          <PositionChangeToken title='Collateral liq.price' token={token} value={balance} value2={balance} ></PositionChangeToken>
        ...
        <div className="flex justify-end py-5  rounded-b-[20px] px-4">
          ...
          <MoreButton className='text-2xl  py-4' text="Confirm" onClick={() => handleRepay()} color="primary" />
        </div>              
      </form>
    </div>
  );
