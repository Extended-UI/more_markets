...
const VaultRepaySet: React.FC<Props> = ({ title, token, balance, apy, ltv, totalRepay, totalTokenAmount, setAmount, closeModal }) => {
  ...
  return (
    <div className='more-bg-secondary w-full  pt-8 rounded-[20px]'>
      <form onSubmit={handleSubmit}>
        <div className="text-2xl mb-10 px-8  pt-5 ">{title}</div>
        ...
        <div className='w-full flex flex-col justify-center px-8 gap-4'>
          ...
          <div className='w-full flex justify-center' >
            <InputTokenMax type="number" value={deposit} onChange={handleInputChange} min="0" max={balanceString}  placeholder={`Repay ${token}`}  token={token} balance={balance}  setMax={handleSetMax}/>
          </div>
